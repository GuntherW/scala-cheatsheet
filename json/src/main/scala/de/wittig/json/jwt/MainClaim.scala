...
def mainClaim(): Unit =
  ...
  var claim = JwtClaim("""{"user":1}""").tap(println)
  ...
  // Stringify as JSON
  claim.tap(println).toJson.tap(println)

  // Manipulate basic attributes
  // Set the issuer
  claim = claim.by("Me").tap(println)

  // Set the audience
  claim = claim.to("You").tap(println)
