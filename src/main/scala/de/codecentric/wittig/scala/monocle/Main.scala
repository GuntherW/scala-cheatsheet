...
object Main extends App {
  ...
  object Employee {
    ...
    val capizalizeStreet: (Employee => Employee) = (Employee.company composeLens Company.address composeLens Address.street composeLens Street.name).modify(_.capitalize)
    def capizalizeStreet2(e: Employee): Employee = (Employee.company composeLens Company.address composeLens Address._street composeLens Street.name).modify(_.capitalize)(e)
  }

  val employee: Employee = Employee(Company(Address(Street("hallo", 33))))

  val neu = (Employee.company composeLens Company.address composeLens Address._street composeLens Street.name).modify(_.capitalize)(employee)
  val neu2 = (Employee.company composeLens Company.address composeLens Address._street composeLens Street.number).modify(_ + 1)(employee)
