...
@main
def simpleForm(): Unit = SimpleFormExample.run()
...
object SimpleFormExample extends LayoutzApp[SimpleFormState, SimpleFormMsg] {
  ...
  def update(msg: SimpleFormMsg, state: SimpleFormState) = msg match {
    ...
    case NextSimpleField          => (state.copy(activeField = (state.activeField + 1) % 2), Cmd.none)
    case SubmitSimple             =>
