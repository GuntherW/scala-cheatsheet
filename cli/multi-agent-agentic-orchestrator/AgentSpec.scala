...
/** Generische, LLM-unabhängige Beschreibung eines Agenten - die Grundlage dafür, dass der Orchestrator generisch (erweiterbar) und agentisch (LLM-planbar) wird.
  ...
  *
  * Ein neuer Agent wird eingebunden, indem lediglich eine neue `AgentSpec`-Instanz der `AgentRegistry` hinzugefügt wird - weder der `Orchestrator` (Execution-Phase) noch der `AgentPlanner`
