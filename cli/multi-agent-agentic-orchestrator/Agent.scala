...
/** Basisklasse für alle Agenten im Multi-Agenten-System.
  ...
  *
  * Der Model-Call selbst läuft über `AnthropicClient.buildAgent` - einen `sttp.ai.core.agent.Agent`-Loop (statt des früheren, direkt in diesem Projekt handgeschriebenen Tool-Use-Loops), inkl.
  * Logging-/Usage-/Budget-Interceptoren.
  */
abstract class Agent(
  ...
  /** Führt den Model-Call aus (bei Bedarf als Multi-Turn Tool-Use-Loop - siehe `AnthropicClient.buildAgent`/`ClaudeToolLoopBackend`). Eine einzige Methode deckt dabei sowohl server-seitige
    * (`useWebSearch`) als auch client-seitige (`clientTools`) Tools ab.
    *
    * Bei einem "unsauberen" Loop-Ende (z. B. Budget-Interceptor greift, oder maximale Iterationen erreicht, bevor eine finale Text-Antwort ohne Tool-Aufruf kam) wird die bestmögliche verfügbare
    * Antwort (`AgentFailure.rawAnswer`) zurückgegeben statt hart zu scheitern - das entspricht dem bisherigen, bewusst nachsichtigen Verhalten dieses Lernprojekts (siehe `AgentConfig`-Systemprompt:
    * "IF THIS IS THE LAST ALLOWED ITERATION, provide your final answer, even if the result is partial").
    */
