...
/** Worker 2: Risk-Analyst
  ...
  * "tool_use"`); WIR führen die Funktion lokal aus und senden das Ergebnis zurück (Multi-Turn-Loop, siehe `AnthropicClient.chat`). Die Rückgabe ist bewusst eine simple Dummy-Berechnung - es geht hier
  * nur darum, Tool-Definition (JSON-Schema) und Ausführung (Handler-Funktion) im Zusammenspiel zu zeigen.
  *
