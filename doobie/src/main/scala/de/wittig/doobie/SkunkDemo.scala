...
class UserRepositoryLive[F[_]: Concurrent: Console](session: Session[F]) extends UserRepository[F]:
  ...
  override def createUser(user: User): F[Unit] =
    for {
      ...
      _        <- Console[F].println(s"Inserted $rowCount rows into the db")
    } yield ()
...
object UserRepositoryLive:
  def apply[F[_]: Concurrent: Console](session: Session[F]): F[UserRepositoryLive[F]] =
    new UserRepositoryLive[F](session).pure[F]
